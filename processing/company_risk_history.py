...
def gc_old_company_risk_snapshots(
    *,
    keep_days: int = 90,
    root: Path | None = None,
    today: date | None = None,
) -> dict:
    """Garbage-collect company-risk snapshot dirs older than ``keep_days``.

    Mirrors :func:`processing.port_supply_history.gc_old_snapshots` — the
    company-risk tree was added alongside the port-supply tree but never
    got a GC job, so it grew one dated dir per day forever. Walks every
    ISO-named subdir under ``root`` (default :data:`COMPANY_RISK_ROOT`)
    and deletes those older than ``today - keep_days``. An operator-
    artefact guard preserves any dir holding a file other than
    ``_FILENAME`` — we never delete data we did not write. Never raises;
    a per-dir failure is swallowed so one bad dir can't abort the sweep.
