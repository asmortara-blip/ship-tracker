...
def _render_quote_board() -> None:
    try:
        ...
        st.markdown(
            f'<div style="background:{C_CARD};border:1px solid {C_BORDER};border-radius:6px;'
            f'padding:16px 20px;">{table_html}</div>', unsafe_allow_html=True)
    except Exception as exc:
...
def _render_options_table() -> None:
    try:
        ...
        st.markdown(
            f'<div style="background:{C_CARD};border:1px solid {C_BORDER};border-radius:6px;padding:16px 20px;">{table_html}</div>', unsafe_allow_html=True)
    except Exception as exc:
