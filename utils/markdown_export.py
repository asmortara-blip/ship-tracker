...
from utils.disclosure import MODELED_NOTICE, assert_disclosed
...
def report_to_markdown(report_data: Any) -> str:
    ...
    # Mandatory not-advice / modeled-provenance disclosure — this artifact is
    # built to travel outside the app, where the in-app banners don't follow.
    parts.append(f"\n> _{MODELED_NOTICE}_\n")

    md = "\n".join(parts)
    # Positive gate: fail loudly rather than ship a modeled report undisclosed.
    assert_disclosed(md)
    return md
